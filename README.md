...
| Latest record | **ROW_NUMBER() OVER** (PARTITION BY id ORDER BY date DESC) → WHERE rn = 1 |  | Manual sort → keep first record |
...
| [Look up across tables](#-a-special-note-on-excel-lookup-and-sql-join)  | JOIN (or subquery) [Troubleshoot my JOIN](#-quick-help-for-join-fanout)  |   | XLOOKUP/VLOOKUP  |
